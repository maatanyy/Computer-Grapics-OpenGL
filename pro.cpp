...
- 모델링 된 객체가 하는 기능은 각자 스스로 정의하도록 함.
    ...
    (C)는 궁극기입니다...뚱이의 키가 커지고 무기가 커지며 입에 문 무기를 휘두릅니다 
...
int legskill1 = 0, legskill2 = 0;
int rot3 = 0;
GLdouble CameraX = 0.0, CameraY = 0.0, CameraZ = 0.0, FocusX = 0.0, FocusY = 0.0, FocusZ = 0.0;
...
void Arrow(GLfloat length)
{
    GLfloat radius = length / 30.0f;
    GLfloat radhead = radius * 2.0f;
    GLint   slices = 20;
    ...
    gluDisk(obj, 0.0, radius, slices, 1);
    gluQuadricOrientation(obj, GLU_OUTSIDE);
    gluCylinder(obj, radius, radius, length - length / 10.0f, slices, 1);
    glTranslatef(0.0f, 0.0f, length - length / 10.0f);
    gluCylinder(obj, radhead, 0.0, length / 10.0f, slices, 1);
    gluDisk(obj, 0.0, radhead, slices, 1);
    glPopMatrix();
...
void DrawAxis()
{
    GLfloat length = 3.0f;  //화살 축의 길이
    //fn = dglCreateFont2D(wglGetCurrentDC(), 20, FW_REGULAR, 0, ANSI_CHARSET, "Courier New");

    glPushMatrix();
    glRotatef(90.0f, 0.0f, 1.0f, 0.0f);
    glColor3f(1.0, 0.0, 0.0);  //x-coordinate : RED
    glMaterialfv(GL_FRONT, GL_AMBIENT, red); 
    Arrow(length);
    // if (fn) dglPrintf2D(fn, 0.0f, 0.0f, length + 0.02f, 0, "x");
    glPopMatrix();

    glPushMatrix();
    glRotatef(-90.0f, 1.0f, 0.0f, 0.0f);
    glColor3f(0.0, 0.0, 1.0);  //y-coordinate : GREEN
    glMaterialfv(GL_FRONT, GL_AMBIENT, purple);  
    Arrow(length);
    // if (fn) dglPrintf2D(fn, 0.0f, 0.0f, length + 0.02f, 0, "y");
    glPopMatrix();

    glColor3f(0.0, 0.0, 1.0);  //z-coordinate : BLUE
    glMaterialfv(GL_FRONT, GL_AMBIENT, blue);  
    Arrow(length);
    //if (fn) dglPrintf2D(fn, 0.0f, 0.0f, length + 0.02f, 0, "z");
}
...
void Body(void) {  //로봇의 몸에 관한 함수입니다.
    ...
    //DrawAxis();
    glPushMatrix();  //PUSH
    glMaterialfv(GL_FRONT, GL_AMBIENT, pink);  //물체의 특성 정의,  주변광계수 sky로
...
void Rightarm(void) {  //오른팔에 관한 함수입니다
    ...
    glMaterialfv(GL_FRONT, GL_AMBIENT, pink);  //물체의 특성 정의,  주변광계수 red로
    ...
    glPushMatrix();
    glMaterialfv(GL_FRONT, GL_AMBIENT, pink);  //물체의 특성 정의,  주변광계수 purple로
    ...
    if (knife == true) {
        glPushMatrix();
        glTranslatef(0.0, 0, 1.0);  //오른손에 관한 내용입니다 팔꿈치에서 이동시켜주고 색을 설정해주고 WireSphere로 만들어 주었습니다
        glColor3f(1.0, 1.0, 1.0);
...
void Leftarm(void) {  //왼쪽팔에 관한 내용입니다
    ...
    if (knife2 == true) {
        ...
        glColor3f(1.0, 1.0, 1.0);
        glMaterialfv(GL_FRONT, GL_AMBIENT, blue);  //물체의 특성 정의,  주변광계수 blue로
        gluCylinder(qobj, 0.2, 0.05+len2, 3.0+skill2, 40, 8);
    }
    glPopMatrix();
}
...
void Rightleg(void) {  //오른쪽 다리에관한 함수입니다
    ...
    glPushMatrix();
    glMaterialfv(GL_FRONT, GL_AMBIENT, yellow);    //물체의 특성 정의,  주변광계수 red로
...
void NeckAndHead(void) {  //목과 머리에 관한 함수입니다
    ...
    glPushMatrix();
    glMaterialfv(GL_FRONT, GL_AMBIENT, pink);   //물체의 특성 정의,  주변광계수 yellow로
...
void MyDisplay(void) {  //MyDispaly 함수에 관한 내용입니다
    ...
    glPushMatrix();
    glTranslatef(1.0, 1.0, 2.0);   //조명 위치 이동
